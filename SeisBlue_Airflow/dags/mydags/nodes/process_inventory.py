...
def read_hyp(hyp_filepath: str, station_time_dict: Dict) -> List[object]:
    """
    ...
    for line in lines:
        ...
        if blank_line > 1:
            ...
            sta = line[1:6].strip()
            name = network + '.' + sta
            timewindow = [station_time_dict[name] if name in station_time_dict.keys() else None][0]
...
if __name__ == '__main__':
    ...
    hyp_filepath = config['process_inventory']['hyp_filepath']
    database = config['global']['database']
    build = config['process_inventory']['build_database']

    print("Start processing inventory.")
    station_time_dict = get_stations_time_window('/mnt/waveforms')
    inventory = read_hyp(hyp_filepath, station_time_dict)
