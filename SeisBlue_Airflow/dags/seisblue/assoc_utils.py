...
def call_hypocenter(match_candidates):
    ...
    temp_file = os.path.join(temp_filename)
...
def use_hypocenter(temp_filename):
    hypo_result = subprocess.run(
        [os.path.join("/tmp/misc/hyp"), temp_filename],
...
def hypo_search(match_candidates):
    ...
    good_match_candidate = [
        candidate for candidate in match_candidates if candidate.sta in good_station
    ]
    ...
    try:
        ...
        return match_candidates, origin, QA
    try:
        if rms > 50 or erdp > 50 or erln > 50 or erlt > 50:
            print(
                f"rms = {rms}, " f"erdp = {erdp}, " f"erln = {erln}, " f"erlt = {erlt}"
            )
            QA = False
            return match_candidates, origin, QA
    except:
        print(f"rms = {rms}, " f"erdp = {erdp}, " f"erln = {erln}, " f"erlt = {erlt}")
        QA = False
        return match_candidates, origin, QA

    if used_sta_num <= 4 and az_gap >= 350:
        print(f"used_station_count = {used_sta_num} " f"azimuthal_gap = {az_gap}")
        QA = False
        return match_candidates, origin, QA

    match_candidates = [
        candidate for candidate in good_match_candidate if candidate.sta in good_station
    ]
...
def write_sfile(match_candidates, temp_filename):
    ...
    ev.origins.append(
        Origin(time=UTCDateTime(event_time), latitude=0, longitude=0, depth=0)
    )
    for candidate in match_candidates:
        _waveform_id_1 = WaveformStreamID(
            station_code=candidate.sta, channel_code="", network_code=""
        )
...
def time_correction(datetime_list, norm="L2"):
    ...
    if norm == "L1":
        ...
        new_time = datetime_list[0] + datetime.timedelta(seconds=mean_offsets)
    elif norm == "L2":
        median_offsets = np.median(offsets)
        new_time = datetime_list[0] + datetime.timedelta(seconds=median_offsets)
    elif norm == "FA":
